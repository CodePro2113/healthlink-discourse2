...
export const useShareManager = () => {
  ...
  const sharePost = async (postId: string): Promise<boolean> => {
    ...
    // If already shared, don't allow sharing again
    ...
    // Allow re-sharing
      ...
      // In a real app, you would call an API to share the post here
      // For example: await supabase.from('shares').insert({ user_id: user.id, post_id: postId });
